...
public sealed class TrackMetadata
{
    ...
    /// </summary>
    /// <remarks>
    /// On the state surfaced by <see cref="Client.ISendspinClient.CurrentGroup"/> and
    /// <see cref="Client.ISendspinClient.GroupStateChanged"/>: each <c>server/state</c> metadata
    /// object is the role's full state (spec #175), so a metadata object that omits the progress
    /// field leaves progress unset (null) — the position is cleared rather than carried forward —
    /// and one carrying the field yields a freshly deserialized <see cref="PlaybackProgress"/>
    /// instance every time. This applies only to the client's state, not to instances deserialized
    /// elsewhere.
    /// </remarks>
