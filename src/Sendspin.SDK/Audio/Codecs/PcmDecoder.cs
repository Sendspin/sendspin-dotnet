...
public sealed class PcmDecoder : IAudioDecoder
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="PcmDecoder"/> class with a logger.
    /// </summary>
    /// <param name="format">Audio format configuration.</param>
    /// <param name="logger">Optional logger for diagnostic output.</param>
    /// <exception cref="ArgumentException">Thrown when format is not PCM.</exception>
    /// <remarks>
    /// Internal rather than an optional parameter on the public constructor: 9.x's surface is
    /// frozen, so the logger reaches the decoder through <see cref="AudioDecoderFactory"/>
    /// without widening the published signature.
    /// </remarks>
    internal PcmDecoder(AudioFormat format, ILogger? logger)
    {
