...
/// When the policy selects the one-shot snap this reports a neutral correction — mode
/// <see cref="SyncCorrectionMode.None"/>, rate 1.0, no stepping: the buffer applies that snap
/// itself on both read paths, and a caller adding its own on top would double-correct.
/// </remarks>
public sealed class SyncCorrectionCalculator : ISyncCorrectionProvider
{
    ...
    public SyncCorrectionCalculator(SyncCorrectionOptions? options, int sampleRate, int channels)
        : this(options, sampleRate, channels, null)
    {
    }

    /// <summary>
    /// Initializes a new instance of the <see cref="SyncCorrectionCalculator"/> class with a logger.
    /// </summary>
    /// <param name="options">Sync correction options. Uses <see cref="SyncCorrectionOptions.Default"/> if null.</param>
    /// <param name="sampleRate">Audio sample rate in Hz (e.g., 48000). Must be greater than zero.</param>
    /// <param name="channels">Number of audio channels (e.g., 2 for stereo). Must be greater than zero.</param>
    /// <param name="logger">
    /// Optional logger, used only to warn when <c>MaxSpeedCorrection</c> exceeds the spec's cap
    /// and is therefore being clamped. Without one the clamp is silent.
    /// </param>
    /// <remarks>
    /// Internal rather than an optional parameter on the public constructor: 9.x's published
    /// surface is frozen, so the logger reaches the calculator without widening the published
    /// signature.
    /// </remarks>
    /// <exception cref="ArgumentOutOfRangeException">
    /// Thrown when <paramref name="sampleRate"/> or <paramref name="channels"/> is less than or equal to zero.
    /// </exception>
    internal SyncCorrectionCalculator(
