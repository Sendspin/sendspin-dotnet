...
public static class Commands
{
    ...
    /// </summary>
    /// <remarks>
    /// Accepted inbound only, and internal because 9.x's published surface is frozen. No server
    /// has adopted the rename yet — aiosendspin still sends <c>set_static_delay</c> — so what
    /// this SDK puts on the wire is unchanged: the player's advertised
    /// <c>supported_commands</c> entry stays <see cref="SetStaticDelay"/> and client/state still
    /// reports <c>static_delay_ms</c>.
    /// </remarks>
    internal const string SetOutputDelay = "set_output_delay";
}
