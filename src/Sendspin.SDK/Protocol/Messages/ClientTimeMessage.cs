...
public sealed class ClientTimeMessage : IMessageWithPayload<ClientTimePayload>
{
    ...
    /// <c>ITimeProbeTransport.SendTimeMessageAsync</c> where the transport supports it, which
    /// stamps T1 at the send point instead, so serialization and send-queue latency stay out of
    ...
    /// <remarks>
    /// For the transport, which stamps T1 at the send point and then builds the message around
    /// it. Internal because 9.x's published surface is frozen.
    /// </remarks>
    internal static ClientTimeMessage Create(long clientTransmitted)
    {
