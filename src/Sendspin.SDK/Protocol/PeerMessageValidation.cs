...
/// <c>"payload": null</c> writes the null straight over it. <c>required</c> does not save them
/// either — it demands presence, not a value. Every non-nullable reference member on the inbound
/// surface is therefore nullable in practice, and an authenticated peer can pick any of them.
/// Checking here is what makes those annotations load-bearing rather than decorative.
/// </para>
/// <para>
...
/// frameworks accept different input. This is hand-rolled instead and behaves identically on both.
/// </para>
/// <para>
...
internal static class PeerMessageValidation
{
    ...
    /// the receive path routes <see cref="JsonException"/> to "malformed input from an
    ...
    internal static void ThrowIfNullMembers(IMessage message)
    {
        switch (message)
        {
            ...
            case ServerStateMessage m:
                // Payload only. Its role objects carry both absent and present-null as meaningful
                // protocol states ("no change" and "clear this role's state"), so neither may be
                // rejected here — only the payload itself, which the model declares non-nullable
                // and HandleServerState dereferences immediately.
                Require(m.Payload, "server/state", "payload");
                break;

            default:
                // Only client-authored messages reach this: from tests round-tripping their own
                // output. Neither is peer input on this client's receive path — the SDK built
                // every member — so there is nothing to police.
                break;
        }
    }
