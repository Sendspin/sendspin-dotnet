...
/// created from the configured <see cref="SourceSupport.Codec"/> when set, and fall back
/// to the capture device's own format when it is not.
/// </summary>
public class SourceCodecSelectionTests
{
    ...
    private static (SendspinClientService Client, FakeSendspinConnection Connection, FakeCaptureDevice Capture) CreateSourceClient(
        Sendspin.SDK.Client.SourceSupport? sourceSupport)
    {
    ...
    public void ConfiguredCodec_OverridesPcmCaptureFormat()
    {
        ...
        // SourceSupport.Codec says, so this fails on the current code. If this test were the
        // only one present, a factory that ignores SourceSupport.Codec entirely (i.e. the
        // pre-fix behaviour) would simply keep failing here forever with no way to prove a
        // real fix was made rather than a hard-coded "always opus" shortcut.
        var (client, connection, capture) = CreateSourceClient(new Sendspin.SDK.Client.SourceSupport { Codec = "opus" });
        using var _c = client;
    ...
    public void UnsetCodec_FallsBackToCaptureFormat()
    {
        ...
        // the encoder to opus regardless of SourceSupport.Codec would still pass it; this is
        // the test that would catch that and only this one does.
        var (client, connection, capture) = CreateSourceClient(new Sendspin.SDK.Client.SourceSupport { Codec = null });
        using var _c = client;
