...
public class TimedAudioBufferClockDriftTests
{
    ...
    {
        // The point of the drift term is that post-anchor offset movement must reach the
        // corrector at all. It now also gets closed on this path: ReadRaw still applies no
        // CONTINUOUS correction (that is the external corrector's job), but the one-shot snap
        // is a buffer-timeline operation the buffer owns on both paths - an external corrector
